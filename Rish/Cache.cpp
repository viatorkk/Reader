...
static BOOL IsLegacyLatinDefaultFont(const LOGFONT* lf)
{
    return lf->lfCharSet == ANSI_CHARSET
        && (0 == _tcsicmp(lf->lfFaceName, _T("Segoe UI Light"))
            || 0 == _tcsicmp(lf->lfFaceName, _T("Segoe UI")));
}

static BOOL IsChineseReadingFontAtPointSize(const LOGFONT* lf, int pointSize)
{
    LOGFONT expected;
    SetChineseReadingFont(&expected, pointSize);
...
static void NormalizeChineseFonts(header_t* header)
{
    ...
    {
        header->font.lfCharSet = DEFAULT_CHARSET;
    }
    ...
    for (int i = 0; i < MAX_TAG_COUNT; i++)
    {
        ...
        {
            header->tags[i].font.lfCharSet = DEFAULT_CHARSET;
        }
    }
#endif
}
