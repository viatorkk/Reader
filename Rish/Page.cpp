...
void Page::DrawPage(HWND hWnd, HDC hdc, RECT* rc, BOOL enable_alpha, alpha_dc_info_t *p_external_alpha_dc)
{
    int i, j, x, y;
    line_info_t* p_line;
    char_info_t* p_char;
    alpha_dc_info_t alpha_dc = {0};
    alpha_dc_info_t *p_alpha_dc = &alpha_dc;
    BOOL release_alpha_dc = FALSE;
    ...
    if (enable_alpha)
    {
        if (p_external_alpha_dc)
        {
            memset(p_external_alpha_dc, 0, sizeof(alpha_dc_info_t));
            p_alpha_dc = p_external_alpha_dc;
        }
        else
        {
            release_alpha_dc = TRUE;
        }
        CreateAlphaTextBitmap(hdc, rc->right - rc->left, rc->bottom - rc->top, p_alpha_dc);
    }
    ...
    if (DrawCover(hdc, rc))
    {
        ...
        {
            if (release_alpha_dc)
                DeleteAlphaTextBitmap(hdc, p_alpha_dc);
            m_BlankPage = FALSE;
        }
        return;
    }
    ...
    for (i = 0; i < m_PageInfo.lines.used; i++)
    {
        ...
        for (j = 0; j < p_line->char_cnt; j++)
        {
            ...
            if (enable_alpha)
            {
                DrawAlphaText(hdc, p_char, x, y, p_line->cy, p_alpha_dc);
            }
            else
            {
    ...
    if (enable_alpha && release_alpha_dc)
    {
        DeleteAlphaTextBitmap(hdc, p_alpha_dc);
    }
    EndDraw();
...
void Page::ReleaseAlphaTextBitmap(HDC hdc, alpha_dc_info_t *p_alpha_dc)
{
    DeleteAlphaTextBitmap(hdc, p_alpha_dc);
}
...
void Page::CreateAlphaTextBitmap(HDC hdc, int width, int height, alpha_dc_info_t *p_alpha_dc)
{
    ...
    if (!p_alpha_dc || width <= 0 || height <= 0)
        return;
    ...
    if (p_alpha_dc->hDIB && p_alpha_dc->pvBits)
    {
        memset(p_alpha_dc->pvBits, 0, width * height * 4);
        p_alpha_dc->hOldBitmap = SelectObject(hdc, p_alpha_dc->hDIB);
    }
}
...
void Page::DeleteAlphaTextBitmap(HDC hdc, alpha_dc_info_t *p_alpha_dc)
{
    if (p_alpha_dc)
    {
        if (p_alpha_dc->hOldBitmap)
            SelectObject(hdc, p_alpha_dc->hOldBitmap);
        if (p_alpha_dc->hDIB)
...
void Page::DrawAlphaText(HDC hdc, char_info_t* p_char, int x, int y, int h, alpha_dc_info_t *p_alpha_dc)
{
    ...
    BYTE textAlpha = m_header && m_header->opaque_text_transparent_bg ? 0xFF : _textAlpha;
    ...
    if (!p_alpha_dc || !p_alpha_dc->pvBits || p_alpha_dc->width <= 0 || p_alpha_dc->height <= 0)
        return;

    top = p_alpha_dc->height - y - (h - p_char->cy) - 1;
    bottom = p_alpha_dc->height - y - h;
    left = x;
    right = x + p_char->cx;
    if (top >= p_alpha_dc->height)
        top = p_alpha_dc->height - 1;
    if (bottom < 0)
        bottom = 0;
    if (left < 0)
        left = 0;
    if (right > p_alpha_dc->width)
        right = p_alpha_dc->width;
    ...
    for (i = top; i >= bottom; i--)
    {
        for (j = left; j < right; j++)
        {
            ...
            else
            {
                ...
                alphaValue = ((*DataPtr) * textAlpha + 127) / 255;
                ThisA = (BYTE)alphaValue; // Move alpha and pre-multiply with RGB
                *DataPtr++ = (BYTE)((FillB * alphaValue + 127) / 255);
                *DataPtr++ = (BYTE)((FillG * alphaValue + 127) / 255);
                *DataPtr++ = (BYTE)((FillR * alphaValue + 127) / 255);
...
{
    int i;
    if (m_drawDC && m_hOldFont)
        SelectObject(m_drawDC, m_hOldFont);
#if ENABLE_TAG
...
_completed:
    if (m_dcIndex != dc_index)
    {
        m_dcIndex = dc_index;
        if (!m_hOldFont)
            m_hOldFont = SelectObject(hdc, m_dcList[m_dcIndex].hFont);
        else
