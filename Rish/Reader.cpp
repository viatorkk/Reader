...
#define TRANSPARENT_BG_HIT_ALPHA    0x01
...
static BYTE GetEffectiveWindowAlpha(void)
{
    if (!_header)
        return 0xFF;
    return _header->alpha < MIN_ALPHA_VALUE ? MIN_ALPHA_VALUE : _header->alpha;
}
...
static BOOL EnsureLayeredWindow(HWND hWnd)
{
    ...
    return (GetWindowLongPtr(hWnd, GWL_EXSTYLE) & WS_EX_LAYERED) == WS_EX_LAYERED;
}

static void ApplyWindowTransparency(HWND hWnd)
{
    if (!_header || _WndInfo.bLayered)
        return;
    if (!EnsureLayeredWindow(hWnd))
        return;
...
VOID OnDraw(HWND hWnd)
{
    ...
    if (image)
    {
        ...
        hOldBitmapBg = (HBITMAP)SelectObject(memdc, hbitmap_bg);
    }
    else
    {
    ...
    { 
        ...
        _Book->ReleaseAlphaTextBitmap(hdc_text, &alpha_dc);
    ...
    if (hOldBitmapBg)
        SelectObject(memdc, hOldBitmapBg);
...
LRESULT OnHideBorder(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    if (_WndInfo.status == ds_normal) // hide border
    {
        ...
        ShowWindow(_WndInfo.hStatusBar, SW_HIDE);
        SetMenu(hWnd, NULL);
        SetWindowPos(hWnd, NULL, rcWin.left, rcWin.top, rcWin.right-rcWin.left, rcWin.bottom-rcWin.top, SWP_FRAMECHANGED | SWP_NOACTIVATE | SWP_NOZORDER);
...
LRESULT OnOpaqueTextTransparentBg(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    _header->opaque_text_transparent_bg = !_header->opaque_text_transparent_bg;
    Save(hWnd);
    Invalidate(hWnd, TRUE, FALSE);
    return 0;
}
...
LRESULT OnWereadRefresh(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    RefreshWereadWebView(hWnd);
    return 0;
}
...
BOOL Init(void)
{
    ...
    for (int i=0; i<_header->item_count; i++)
    {
        ...
        if (!PathFileExists(item->file_name)
            ...
            || 0 == _tcscmp(PathFindExtension(item->file_name), _T(".ol"))
#endif
            )
        {
            _Cache.delete_item(i);
