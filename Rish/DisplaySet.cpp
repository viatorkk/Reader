...
{
    int percent = ((int)alpha * 100 + 127) / 255;
    if (percent < MIN_ALPHA_PERCENT)
        return MIN_ALPHA_PERCENT;
    if (percent > 100)
        return 100;
    return percent;
}

static BYTE _percent_to_alpha(int percent)
{
...
void OpenDisplaySetDlg(void)
{
    ...
    if (_display.is_save)
    {
        ...
        UpdateWereadWebViewBackground(_hWnd);
        Save(_hWnd);
...
static void _init_layout_set(HWND hDlg)
{
    ...
    SetDlgItemInt(hDlg, IDC_EDIT_ALPHA, _alpha_to_percent(_display.alpha), FALSE);
...
static BOOL _valid_layout_set(HWND hDlg)
{
    ...
    if (!bResult || _display.char_gap < 0)
    {
    ...
    if (!bResult || _display.line_gap < 0)
    {
    ...
    if (!bResult || _display.paragraph_gap < 0)
    {
    ...
    if (4 != _stscanf(buf, _T("%d,%d,%d,%d"), &_display.internal_border.left, &_display.internal_border.top, &_display.internal_border.right, &_display.internal_border.bottom)
        || _display.internal_border.left < 0 || _display.internal_border.top < 0
        || _display.internal_border.right < 0 || _display.internal_border.bottom < 0)
    {
        MessageBox_(hDlg, IDS_INVALID_INBORDER, IDS_ERROR, MB_OK|MB_ICONERROR);
        return FALSE;
    }
    int alpha_percent = GetDlgItemInt(hDlg, IDC_EDIT_ALPHA, &bResult, FALSE);
    if (!bResult || alpha_percent < MIN_ALPHA_PERCENT || alpha_percent > 100)
    {
        MessageBox_(hDlg, IDS_INVALID_VALUE, IDS_ERROR, MB_OK|MB_ICONERROR);
        return FALSE;
    }
    _display.alpha = _percent_to_alpha(alpha_percent);
    return TRUE;
}
...
static void _update_preview(HDC hDC, RECT *p_rc)
{
    ...
    if (!_display.use_same_font)
    {
        hOldFont = (HFONT)SelectObject(memdc, hCptFont);
