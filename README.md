...
1. 修复打开、切换和关闭书籍时的加载线程回收问题，避免强制终止线程带来的崩溃风险。
2. 修复章节正则空匹配导致的卡住，支持取消长文本扫描，并忽略过期的加载结果。
3. 加强 TXT 编码、文件大小和内存分配的边界检查，修复 EPUB 连续章节缺少标题时的内存问题。
4. 新增 201 项阅读加载回归检查及 TXT 打开、响应、退出和重启恢复测试。
